...
const EducationSection = () => {
  const education = [
    ...
    },
    {
      ...
      major: 'Science',
      institution: 'Mudaforganj Ali Nowab High School & College',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {certifications.map((cert, index) => (
              <div key={index} className="group [perspective:1000px]">
                <div className="relative h-40 w-full [transform-style:preserve-3d] transition-transform duration-500 group-[.is-flipped]:[transform:rotateY(180deg)]">
                  {/* Front */}
                  <Card className="absolute inset-0 p-4 bg-surface border-card-border hover:shadow-glow [backface-visibility:hidden]">
                    <div className="flex items-center gap-2 mb-3">
                      <div className={`w-2 h-2 rounded-full ${cert.status === 'Completed' ? 'bg-green-400' : 'bg-yellow-400'}`}></div>
                    ...
                    <div className="flex gap-2">
                      <button className="flex-1 text-xs px-3 py-1 bg-primary/20 text-primary rounded hover:bg-primary/30 transition-colors" onClick={(e) => {
                  ...
                  {/* Back */}
                  <Card className="absolute inset-0 p-4 bg-surface border-card-border text-xs text-muted-foreground [transform:rotateY(180deg)] [backface-visibility:hidden]">
                    <h4 className="font-semibold text-sm mb-2 text-foreground">About this certification</h4>
                    <p>
                      {cert.name} — issued by {cert.issuer}. Validates practical skills relevant to real-world data projects.
                    </p>
                    <button className="mt-3 text-xs px-3 py-1 bg-primary/20 text-primary rounded hover:bg-primary/30 transition-colors" onClick={(e) => {
                      const card = (e.currentTarget.closest('.group')) as HTMLElement | null;
