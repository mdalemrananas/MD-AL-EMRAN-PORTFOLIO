            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
              {skillDistribution.map((item) => (
                <div key={item.name} className="flex items-center gap-2 min-w-0">
                  <span className="inline-block w-3 h-3 rounded flex-shrink-0" style={{ backgroundColor: item.color }}></span>
            ...
            <div className="space-y-3">
              {toolsPlatforms.map((skill, index) => (
                <div key={index} className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="text-sm text-foreground truncate" title={skill}>{skill}</span>
                </div>
              ))}
            </div>
          </Card>
